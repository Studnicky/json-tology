...
// ---------------------------------------------------------------------------
  ...
  // Canonical TBox projection lives in OwlProjection; its tests are in
  // test/unit/owlProjection.test.ts.

  // ---------------------------------------------------------------------------
  // JsonLdFormatter.fromQuads — canonical quad -> JSON-LD conversion
  // ---------------------------------------------------------------------------

  void describe('JsonLdFormatter.fromQuads', () => {
    const scenarios: Array<{
      'check': (nodes: Array<Record<string, unknown>>) => void;
      'name': string;
      'quads': QuadInterface[];
    }> = [{
      // JsonLdFormatter.fromQuads inlines singly-referenced blank nodes:
      // _:b1 is referenced once by ex:child so it is inlined into root,
      // producing root['ex:child'] = { @type, ex:value } without the @id key.
      // _:b1 does not appear as a separate top-level node.
      // No double-prefixing _:_:b1 (the blank-node value is the raw label).
      'check': (nodes) => {
        ...
        // _:b1 is inlined — no separate top-level bnode entry
        const separate = nodes.find((node) => {
          return node['@id'] === '_:b1';
        });

        assert.equal(separate, undefined, 'singly-referenced blank node must be inlined');
        // Inlined content has no @id key
        const inlined = root['ex:child'];

        assert.ok(inlined !== null && typeof inlined === 'object', 'ex:child must be an inlined object');
        const inlinedObj = inlined as Record<string, unknown>;

        assert.ok(!('@id' in inlinedObj), 'inlined bnode must not carry @id');
        // No double-prefixing
        assert.equal(JSON.stringify(nodes).includes('_:_:b1'), false, 'no double-prefixed blank node label');
      },
      'name': 'inlines singly-referenced blank nodes and avoids double-prefixing',
      'quads': [
        Terms.quad(
