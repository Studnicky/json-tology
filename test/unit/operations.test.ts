...
import { applyOp, clone } from '../../src/modules/data/operations.js';
...
void describe('applyOp()', () => {
  void it('set at root replaces the whole value', () => {
    const result = applyOp({ a: 1 }, { op: 'set', path: '/', value: { b: 2 } });

    assert.deepEqual(result, { b: 2 });
  });

  void it('del at root returns undefined', () => {
    const result = applyOp({ a: 1 }, { op: 'delete', path: '/' });
  ...
  void it('set at a single-segment path sets property on object', () => {
    const result = applyOp({ x: 1 }, { op: 'set', path: '/x', value: 99 }) as Record<string, unknown>;

    assert.equal(result.x, 99);
  });

  void it('del at a single-segment path removes property', () => {
    const result = applyOp({ x: 1, y: 2 }, { op: 'delete', path: '/x' }) as Record<string, unknown>;
  ...
  void it('does not mutate the original object', () => {
    ...
    applyOp(original, { op: 'set', path: '/a/b', value: 2 });
  ...
  void it('does not mutate the original array', () => {
    ...
    applyOp(original, { op: 'delete', path: '/items/0' });

    assert.deepEqual(original.items, [1, 2, 3]);
  });

  void it('set adds a new property on an object', () => {
    const result = applyOp({ a: 1 }, { op: 'set', path: '/b', value: 2 }) as Record<string, unknown>;
