...
**Status:** implemented.
**Parent:** `designs/0005` §7 (this is the proper-architecture expansion of "lazy `TMap`").
...
re-aliasing of canonical types that `CLAUDE.md` / designs/0005 §8 forbid. Each
...
- **`dump`/`encode` return the wire `InputType`** (`LooseInputType<…>`),
  consistent with designs/0005 §8 — never `unknown`.
...
## Locked principle (from designs/0005 "Guiding principle")
...
- **#1 (designs/0005 §1)** — `src/types/Infer.ts` `InferRefType`: the unresolved
...
/** Brand-free wire INPUT for dump/encode (the InputType of designs/0005 §8). */
...
| `dump` (~1022–1032) | **both** overloads return `unknown`. | Return the wire `ResolveInputType<T, TRefs>` (designs/0005 §8 InputType); `value` param typed `ResolveOutputType<T, TRefs>`. |
...
4. `dump` / `encode` — return `ResolveInputType` (wire/InputType, designs/0005 §8).
...
9. Update `designs/0005` §7 to "DONE — single precise signatures, no `unknown`
