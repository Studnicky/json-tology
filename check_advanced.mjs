...
const jt = new JsonTology({ 
  ...
  schemas: [Animal, Pet, Status, Config] 
});

console.log('=== JSON-LD - Status with enum ===');
const jsonld = jt.ontology().jsonLdObject();
const status = jsonld['@graph'].find(n => n['@id'] === 'https://myapp.io/Status');
console.log(JSON.stringify(status, null, 2));

console.log('\n=== JSON-LD - Pet with oneOf ===');
const pet = jsonld['@graph'].find(n => n['@id'] === 'https://myapp.io/Pet');
console.log(JSON.stringify(pet, null, 2));

console.log('\n=== JSON-LD - Config with const ===');
const config = jsonld['@graph'].find(n => n['@id'] === 'https://myapp.io/Config');
