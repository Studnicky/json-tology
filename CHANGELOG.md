...
- Bookstore demo extended with five new entities — `EBookSchema` and `PrintBookSchema` (disjoint subclasses of `BookSchema`), `RareBookSchema` (chained restrictions on `Book#authors`: `someValuesFrom AuthorName` + `maxCardinality 1`), and `PromotionalBookSchema` / `NonPromotionalBookSchema` (the latter declared via `Compose.complementOf`). The `bookstoreEntities` registry also records an `owl:sameAs` ABox assertion between two customer IRIs to demonstrate the identity surface.
- Live ontology graph (`BookstoreGraph` Vue component) renders five additional edge kinds — `disjointWith` (red dashed), `complementOf` (purple with tee terminator), `restriction` (teal dotted, class → constrained property), `sameAs` (gold dashed, symmetric), and existing `subClassOf` / `equivalentClass` — pulled from the OWL TBox quads plus a new `SameAsStore` pass against the registry.
- `bookstore-domain.md` documents the new schemas, their wire shapes, and the OWL TBox/ABox emissions they produce.
