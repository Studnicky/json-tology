 ...
 *
 * Called by {@link JsonTology.prefetch} during transitive `$ref` resolution when a
 * referenced IRI is not yet registered. Returns the parsed schema object for the
 * given IRI, or `null` if the IRI is unknown to this loader. Returning `null` causes
 * {@link GraphError} `REF_UNRESOLVED` to be thrown with the IRI. Network errors
 * should propagate so callers see real connectivity failures.
 */
