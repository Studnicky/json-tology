...
export const SchemaCompilerCheckExec = {
  buildNodeCheckExecution(
    ...
    if (sem.hasConst) {
      checks.push(SchemaCompilerGraph.compileConstCheck(sem.constValue));
    }

    if (sem.enumValues !== undefined) {
      checks.push(SchemaCompilerGraph.compileEnumCheck(sem.enumValues));
    }
