...
export function buildNodeCheckExecution(
  ...
  if (sem.minLength !== undefined || sem.maxLength !== undefined || sem.pattern !== undefined || sem.format !== undefined) {
    // eslint-disable-next-line @stylistic/max-len
    const stringCheck = context.compileStringCheck(sem.minLength, sem.maxLength, sem.pattern, sem.format, formatRegistry, sem);
