...
import { deepEqual, isRecord } from '../data/DataTypes.js';
...
export class SchemaCompiler {
  ...
  private compileCheck(
    ...
    if (Array.isArray(allOf) && allOf.length > 0) {
      const allOfChecks = (allOf as JsonSchemaType[]).map((s) => {
    ...
    if (Array.isArray(anyOf) && anyOf.length > 0) {
      const anyOfChecks = (anyOf as JsonSchemaType[]).map((s) => {
        if (typeof s === 'boolean') return s ? () => true : () => false;
    ...
    if (Array.isArray(oneOf) && oneOf.length > 0) {
      const oneOfChecks = (oneOf as JsonSchemaType[]).map((s) => {
        if (typeof s === 'boolean') return s ? () => true : () => false;
  ...
  private compileObjectCheck(
    ...
    const properties = schema['properties'] as Record<string, JsonSchemaType> | undefined;
    ...
    // Pattern properties
    const patternProperties = schema['patternProperties'] as Record<string, JsonSchemaType> | undefined;
    let patternChecks: Array<{ 'check': CheckFnType; 'regex': RegExp; }> | undefined;
  ...
  private compileArrayCheck(
    ...
    const itemsSchema = schema['items'] as JsonSchemaType | undefined;
    const prefixItems = schema['prefixItems'] as JsonSchemaType[] | undefined;
  ...
  private compileValidateWithErrors(
    ...
    const allOf = schema['allOf'] as JsonSchemaType[] | undefined;
    ...
    if (Array.isArray(allOf) && allOf.length > 0) {
      allOfValidators = allOf.map((s) => {
        if (typeof s === 'boolean') {
          return s
            ? (_v: unknown, _p: string, _e: ValidationErrorType[], _c: boolean, _ad: boolean, _dc: boolean, _su: boolean) => ({ 'valid': true, 'value': _v })
            : (v: unknown, p: string, e: ValidationErrorType[], c: boolean) => {
