...
export const GraphEngineVisit = {
  visit(
    ...
    if (depth > options.maxSchemaDepth) {
      throw new GraphError(`Maximum schema recursion depth (${options.maxSchemaDepth}) exceeded at path: ${path}`, {
    ...
    if (typeof rdfsRange === 'string') {
      const visitFn = GraphEngineVisit.visit;

      Unevaluated.rdfsRange(context, rdfsRange, workingValue, path, options, refStack, depth, visitFn, pushErrors);
    }
