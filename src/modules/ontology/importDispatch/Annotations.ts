...
export class Annotations {
  ...
  private static applyLabelFields(delta: Record<string, unknown>, acc: AnnotationAccumulatorType): void {
    ...
    const description = Annotations.resolveLangValue(acc.comments);
  ...
  private static buildSchemaDeltas(accumulators: Map<string, AnnotationAccumulatorType>): Map<string, Record<string, unknown>> {
    ...
    for (const [
      ...
      const delta = Annotations.buildDelta(acc);
  ...
  public static dispatch(_quads: QuadInterface[], ctx: OwlImportContextType): OwlImportFragmentType {
    ...
    return {
      ...
      'invariants': [],
      'sameAs': [],
      'schemaDeltas': Annotations.buildSchemaDeltas(accumulators)
    };
  }
  ...
  private static dispatchLiteralRelation(
    ...
    if (COMMENT_PREDICATES.has(predicateIri)) {
      Annotations.applyCommentRelation(relation, acc);
  ...
  private static dispatchRelation(
    ...
    const acc = Annotations.getOrCreateAccumulator(accumulators, subjectIri);

    Annotations.dispatchLiteralRelation(relation, predicateIri, acc);
  ...
  private static getOrCreateAccumulator(
    ...
    const acc = Annotations.makeAccumulator();
