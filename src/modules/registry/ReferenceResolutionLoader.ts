...
export class ReferenceResolutionLoader implements ReferenceResolutionLoaderInterface {
  ...
  public async resolveAll(loader: LoaderType): Promise<void> {
    ...
    const resolveSchema = async (schema: Record<string, unknown>): Promise<void> => {
      const unresolved = this.registry.collectUnresolvedReferenceIris(schema);
