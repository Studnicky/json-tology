...
function isAsciiHostname(value: string): boolean {
  ...
  for (let i = 0; i < value.length; i++) {
    ...
    if (c === 0x2E) { // '.'
      ...
      if (!isAlphanumeric(value.charCodeAt(i - 1))) return false;
      labelLen = 0;
      continue;
    }
    if (labelLen === 0 && !isAlphanumeric(c)) return false;
    if (!isAlphanumeric(c) && c !== 0x2D) return false; // not alphanumeric or '-'
    labelLen++;
...
function validateDate(value: string, offset: number): boolean {
  if (offset + 10 > value.length) return false;
  for (let i = 0; i < 4; i++) if (!isDigit(value.charCodeAt(offset + i))) return false;
  if (value.charCodeAt(offset + 4) !== 0x2D) return false; // '-'
  if (!isDigit(value.charCodeAt(offset + 5)) || !isDigit(value.charCodeAt(offset + 6))) return false;
  if (value.charCodeAt(offset + 7) !== 0x2D) return false;
  if (!isDigit(value.charCodeAt(offset + 8)) || !isDigit(value.charCodeAt(offset + 9))) return false;
...
function validateTime(value: string, offset: number): boolean {
  ...
  const h1 = value.charCodeAt(offset), h2 = value.charCodeAt(offset + 1);

  if (!isDigit(h1) || !isDigit(h2)) return false;
  const hour = (h1 - 0x30) * 10 + (h2 - 0x30);

  if (hour > 23) return false;
  if (value.charCodeAt(offset + 2) !== 0x3A) return false; // ':'
  const m1 = value.charCodeAt(offset + 3), m2 = value.charCodeAt(offset + 4);

  if (!isDigit(m1) || !isDigit(m2)) return false;
  if ((m1 - 0x30) * 10 + (m2 - 0x30) > 59) return false;
  if (value.charCodeAt(offset + 5) !== 0x3A) return false;
  const s1 = value.charCodeAt(offset + 6), s2 = value.charCodeAt(offset + 7);

  if (!isDigit(s1) || !isDigit(s2)) return false;
  if ((s1 - 0x30) * 10 + (s2 - 0x30) > 60) return false; // 60 for leap second
  let pos = offset + 8;

  // fractional seconds
  if (pos < value.length && value.charCodeAt(pos) === 0x2E) {
    pos++;
    if (pos >= value.length || !isDigit(value.charCodeAt(pos))) return false;
    while (pos < value.length && isDigit(value.charCodeAt(pos))) pos++;
  }
...
const STRING_FORMAT_VALIDATORS: Record<string, (value: string) => boolean> = {
  ...
  'duration': (value) => {
    if (value.length < 2 || value.charCodeAt(0) !== 0x50) return false; // 'P'
    let pos = 1;
    ...
    while (pos < value.length) {
      ...
      if (unit === 0x2E && inTime) { // '.' for fractional seconds
        ...
        if (pos >= value.length || !isDigit(value.charCodeAt(pos))) return false;
        while (pos < value.length && isDigit(value.charCodeAt(pos))) pos++;
        if (pos >= value.length || value.charCodeAt(pos) !== 0x53) return false; // 'S'
      }
      pos++;
      hasContent = true;
    }
  ...
  'idn-email': (value) => {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      if (c <= 0x20 || (c === 0x40 && i !== at)) return false; // whitespace or duplicate @
    }
  ...
  'json-pointer': (value) => {
    ...
    for (let i = 1; i < value.length; i++) {
      if (value.charCodeAt(i) === 0x7E) { // '~'
        ...
        if (next !== 0x30 && next !== 0x31) return false; // must be '0' or '1'
      }
    }
  ...
  'time': (value) => {
    ...
    if (pos === value.length) return true; // no timezone is valid
    const tzChar = value.charCodeAt(pos);

    if (tzChar === 0x5A) return pos + 1 === value.length; // 'Z'
    if (tzChar !== 0x2B && tzChar !== 0x2D) return false; // '+' or '-'
    // +HH:MM
    if (pos + 6 !== value.length) return false;
    if (!isDigit(value.charCodeAt(pos + 1)) || !isDigit(value.charCodeAt(pos + 2))) return false;
    if (value.charCodeAt(pos + 3) !== 0x3A) return false;
    if (!isDigit(value.charCodeAt(pos + 4)) || !isDigit(value.charCodeAt(pos + 5))) return false;
  ...
  'uuid': (value) => {
    ...
    if (value.charCodeAt(8) !== 0x2D || value.charCodeAt(13) !== 0x2D ||
        value.charCodeAt(18) !== 0x2D || value.charCodeAt(23) !== 0x2D) return false;
    // Version digit at position 14 must be 1-8
    const version = value.charCodeAt(14);

    if (version < 0x31 || version > 0x38) return false;
    ...
    if (!((variant >= 0x38 && variant <= 0x39) || variant === 0x61 || variant === 0x62 ||
          variant === 0x41 || variant === 0x42)) return false;
 ...
 */
export function builtinFormats(): FormatRegistry {
  const registry = new FormatRegistry();

  for (const [name, fn] of Object.entries(STRING_FORMAT_VALIDATORS)) {
    registry.register(name, (v) => typeof v === 'string' && fn(v));
  }

  for (const [name, fn] of Object.entries(NUMBER_FORMAT_VALIDATORS)) {
    registry.register(name, (v) => typeof v === 'number' && fn(v));
  }
