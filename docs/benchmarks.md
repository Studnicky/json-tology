...
2. **Browser runner** — `▶ Run in browser` button that loads the listed comparators from their `esm.sh` CDN entries on demand and runs the same scenario in your browser. Numbers are directional; browser timing variance is high.
...
A json-tology-internal A/B between `SchemaCompiler` and `GraphEngine.execute` on the same registered schema. No peer library has an equivalent surface — there's nothing to compare against in-browser, only against itself.
